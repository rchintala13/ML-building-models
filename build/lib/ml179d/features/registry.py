...
def apply_base_features(
    ...
    Order matters: add_bldg_volume and add_sa_to_vol_ratio both read
    'roof_area', so add_roof_area must precede them when roof_area is derived
    rather than loaded from the raw CSV.
    """
    for name in names:
