...
def scan_batches(
    ...
    *,
    ...
    strict:
        If True, any CSV under raw_data_dir whose name does not match the batch
        pattern raises. If False, such files are skipped, which is the safer
        choice when the directory also holds exports or notes.
    ...
    for csv_path in csv_files:
        meta = parse_batch_filename(csv_path.name, strict=strict)
        if meta is None:
            continue
    ...
    if not rows:
        raise RuntimeError(
            f"No CSV files under {raw_data_dir} matched the batch filename pattern."
        )
